data = """
park 800905-1049118 park@hamail.com
kim  700905-1059119 kim@naver.com
Life is egg
Lee 700906-1059118 lee@gmail.com
"""
...
for line in data.split("\n"):
    ...
    for word in line.split(" "):
        ...
        #- 뒤의 7자리가 숫자여야 한다. word[7:] 7번부터 마지막까지 전체 숫자로만 구성되어 있느냐
        # word[6]=="-" 조건을 추가할 수도 있다.

        if len(word) == 14 and word[:6].isdigit() and word[7:].isdigit(): #주민번호 패턴이 맞다고 한다면
            word = word[:6] + "-" + "*******"
            #앞자리 6자리 + - + *******
        word_result.append(word) #같은 라인에 여러 개의 주민번호가 있을 수 있음
...
pat = re.compile("(\d{6})[-]\d{7}") #\d 숫자 {6} 여섯자리 [-] \d{7}
